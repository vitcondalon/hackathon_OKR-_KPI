...
export default function EntityCrudPage({
  ...
  const stats = useMemo(
    () => [
      ...
      { label: 'Ðang hi?n th?', value: filteredItems.length, tone: 'text-brand-600' },
      { label: 'Tr?ng thái s?ng', value: inferHighlightValue(items, columns), tone: 'text-emerald-600' }
    ],
  ...
  async function refresh() {
    ...
    try {
      ...
      setError(apiErrorMessage(err, `Không th? t?i d? li?u ${title}`));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function submit(event) {
    ...
    try {
      ...
      setError(apiErrorMessage(err, 'Không th? luu d? li?u'));
    } finally {
      setSaving(false);
    }
  }
  ...
  async function remove(id) {
    if (!window.confirm('B?n có ch?c ch?n mu?n xóa b?n ghi này?')) return;
    try {
      await deleteItem(id);
      if (editingId === id) resetForm();
      await refresh();
    } catch (err) {
      setError(apiErrorMessage(err, 'Không th? xóa b?n ghi'));
    }
  }
    ...
    <div className="space-y-5 ui-page-enter">
      ...
      <div className="grid gap-5 xl:grid-cols-[400px_minmax(0,1fr)]">
        <Card
          ...
          className="h-fit"
          actions={editingId ? <span className="status-badge border-amber-200 bg-amber-50 text-amber-700">Ðang s?a</span> : null}
        >
          {canCreate || canUpdate ? (
            <form onSubmit={submit} className="space-y-4">
                ...
                <label key={field.name} className="block space-y-1.5 text-sm">
                  ...
                  {field.type === 'textarea' ? (
                    ...
                    <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white px-4 py-3">
                      <span className="text-sm text-slate-600">B?t tru?ng này</span>
                      <input
                        type="checkbox"
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                <Button type="submit" disabled={saving}>
                  {saving ? 'Ðang luu...' : editingId ? 'C?p nh?t' : 'T?o m?i'}
                </Button>
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50/80 p-4 text-sm text-slate-600">
              Vai trò hi?n t?i ch? có quy?n xem.
            </div>
          )}
        </Card>
        ...
        <Card
          ...
          subtitle="D? li?u API th?c t? d? theo dõi và thao tác nhanh"
          actions={
        ...
        >
          <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="min-w-0 flex-1">
              <input
                placeholder={`Tìm ${title.toLowerCase()} theo c?t dang hi?n th?...`}
                value={search}
          ...
          {loading ? (
            ...
            <DataTable
              columns={columns}
              data={filteredItems}
              emptyLabel={`Không có b?n ghi ${title.toLowerCase()} phù h?p v?i b? l?c hi?n t?i`}
              actions={
                hasActions
                  ? (row) => (
                      <div className="flex flex-wrap gap-2">
